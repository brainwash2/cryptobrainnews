#!/usr/bin/env ts-node
/**
 * Environment Variable Validator for CryptoBrainNews
...
// ─── Validation logic ─────────────────────────────────────────────────────
function checkVariable(name: string, isRequired: boolean): boolean {
  ...
  if (isRequired && !exists) {
    ...
    return true; // optional missing doesn't fail
  } else if (exists) {
