    ...
    <header className="fixed top-0 left-0 right-0 z-[1000] bg-black border-b-2 border-primary h-16 shadow-2xl">
      <div className="container mx-auto px-4 lg:px-8 h-full flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link 
              key={link.href} 
              href={link.href}
              className={`text-xs font-black tracking-widest transition-colors ${pathname === link.href ? 'text-primary' : 'text-gray-400 hover:text-white'}`}
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-4">
          <Link 
            href="/go-alpha" 
            className="hidden md:block bg-white/10 text-primary border border-primary/50 text-xs font-black px-4 py-2 hover:bg-primary hover:text-black transition-all"
          >
            GO ALPHA
          </Link>
      ...
      {/* MOBILE MENU */}
      {isMenuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-black border-b-2 border-primary p-8 flex flex-col gap-6 lg:hidden animate-in slide-in-from-top duration-300">
          {navLinks.map((link) => (
          ...
          <Link href="/go-alpha" onClick={() => setIsMenuOpen(false)} className="text-primary font-black text-sm tracking-widest">
            GO ALPHA
          </Link>
        </div>
      )}
