...
export default function Header() {
  ...
  useEffect(() => {
    function handleScroll() { setNewsDropdownOpen(false); }
  ...
  return (
    <>
      ...
      <header className="bg-black border-b border-[#222] sticky top-0 z-[200]">
        <div className="max-w-[1600px] mx-auto px-4 lg:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden lg:flex items-center gap-6">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {newsDropdownOpen && (
                  <div className="absolute top-full left-0 mt-3 w-96 bg-black border border-[#333] shadow-[0_8px_32px_rgba(0,0,0,0.8)] z-[300]">
                    {/* Triangle pointer */}
                    <div className="absolute -top-1.5 left-4 w-3 h-3 bg-black border-l border-t border-[#333] rotate-45" />
 
                    <Link
                      href="/news"
                      onClick={() => setNewsDropdownOpen(false)}
                      className="flex items-center gap-2 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-[#FABF2C] hover:bg-[#111] border-b border-[#1a1a1a] transition-colors"
                    >
                      All News
                    </Link>
                    ...
                    <div className="py-1">
                      {NEWS_CATEGORIES.map(cat => (
                        <Link
                          ...
                          onClick={() => setNewsDropdownOpen(false)}
                          className="flex items-center justify-between px-4 py-2 text-[10px] font-black uppercase tracking-widest text-[#888] hover:text-[#FABF2C] hover:bg-[#0a0a0a] transition-colors"
                        >
                          {cat.label}
                        </Link>
                      ))}
                    ...
                    <div className="border-t border-[#1a1a1a]">
                      <Link
                        href="/bookmarks"
                        onClick={() => setNewsDropdownOpen(false)}
                        className="flex items-center gap-2 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-[#555] hover:text-[#FABF2C] hover:bg-[#0a0a0a] transition-colors"
                      >
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <div className="relative flex items-center">
              {searchOpen ? (
                <form onSubmit={handleSearch} className="flex items-center gap-2">
                  ...
                  />
                  <button type="submit" className="text-[#FABF2C] hover:text-white transition-colors">
                    <Search size={14} />
                  </button>
                  <button
                    type="button"
                ...
                <button
                  ...
                  className="text-[#555] hover:text-[#FABF2C] transition-colors p-1"
                  aria-label="Search"
                >
                  <Search size={16} />
                </button>
            ...
            <Link href="/advertise" className="text-[10px] text-[#555] font-black uppercase tracking-widest hover:text-white transition-colors">
              Advertise
            </Link>
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="lg:hidden absolute top-16 left-0 w-full bg-black border-b border-[#222] p-4 flex flex-col gap-4 shadow-2xl max-h-[80vh] overflow-y-auto z-[300]">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                const q = searchQuery.trim();
            ...
            <div>
              ...
              {NEWS_CATEGORIES.map(cat => (
                <Link key={cat.slug} href={`/news/category/${cat.slug}`} onClick={() => setIsOpen(false)}
                  className="block text-xs font-black uppercase tracking-widest text-[#888] hover:text-white py-2">
                  {cat.label}
                </Link>
              ))}
              <Link href="/bookmarks" onClick={() => setIsOpen(false)} className="block text-xs font-black uppercase tracking-widest text-[#555] hover:text-white py-2">
                🔖 Saved Articles
              </Link>
            </div>
