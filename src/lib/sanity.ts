  ...
  return cached(`sanity:author:${slug}`, async () => {
    return client.fetch(
      `*[_type == "author" && slug.current == $slug][0] {
        ...
        bio, role, "avatarUrl": avatar.asset->url, twitter
      }`,
      { slug } as QueryParams
    );
  }, 3600);
...
export async function getPostsByAuthor(authorSlug: string) {
  return cached(`sanity:posts:author:${authorSlug}`, async () => {
    return client.fetch(
      ...
      { slug: authorSlug } as QueryParams
    );
  }, 300);
}
...
export async function getPostsByTag(tag: string) {
  return cached(`sanity:posts:tag:${tag}`, async () => {
    return client.fetch(
  ...
  return cached('sanity:events', async () => {
    return client.fetch(`
      *[_type == "event" && endDate >= now()] | order(startDate asc) {
        ...
        venue, url, isOnline, isFeatured, description,
...
export async function getSanityGlossary() {
  return cached('sanity:glossary', async () => {
    return client.fetch(
      `*[_type == "glossaryTerm"] { _id, term, definition, category }`
    );
  }, 3600);
}
