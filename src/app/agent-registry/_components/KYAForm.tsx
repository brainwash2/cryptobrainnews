...
export default function KYAForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate on-chain registration delay
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // Generate simulated API key
    const generatedKey = `cbn_live_${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}`;
    ...
    return (
      <div className="bg-[#0a0a0a] border border-[#00d672]/30 p-8 text-center rounded">
        ...
        <p className="text-xs text-[#888] font-mono mb-6">
          Identity verified. Your Oracle API key is ready for deployment.
        </p>
        
        <div className="bg-black border border-[#222] p-4 flex items-center justify-between gap-4 rounded mb-6">
          <code className="text-[#00d672] text-xs font-mono break-all text-left">
            {apiKey}
          </code>
        ...
        <div className="bg-[#111] border border-[#222] p-4 text-left rounded">
          <p className="text-[10px] font-black text-[#555] uppercase tracking-widest mb-2 flex items-center gap-2">
            <Terminal size={12} /> Usage Example
          </p>
          <pre className="text-[#ccc] text-[10px] font-mono overflow-x-auto">
            curl -H "Accept: application/json" \<br/>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-H "Authorization: Bearer {apiKey}" \<br/>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;https://cryptobrainnews.com/api/oracle/airdrops
          </pre>
        </div>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-[#0a0a0a] border border-[#1a1a1a] p-8 rounded">
      <div>
        <label className="block text-[10px] font-black uppercase text-[#555] tracking-widest mb-2">
          Agent Identifier
        </label>
        <input
          required
          type="text"
          placeholder="e.g. OpenClaw-Alpha-01"
      ...
      <button
        ...
        className="w-full bg-[#FABF2C] text-black py-4 text-xs font-black uppercase tracking-widest hover:bg-white transition-all disabled:opacity-50 mt-4"
      >
