...
// Helper to generate simulated historical trend based on current live data
function generateSimulatedTrend(currentVol: number, currentOi: number) {
  return Array.from({ length: 30 }, (_, i) => {
    const day = new Date();
    day.setDate(day.getDate() - (29 - i));
    
    // Create a natural-looking curve anchoring the last day to the real current value
    const variance = Math.sin(i / 3) * 0.1 + (Math.random() * 0.05 - 0.02);
    ...
    <div className="space-y-12 pb-20">
      ...
      <div className="grid grid-cols-1 gap-8">
        <BlockChartCard 
          ...
          description="Simulated historical trend based on current live DefiLlama aggregates."
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          ...
          <div className="border border-[#1a1a1a] bg-[#0a0a0a]">
            <DataTable
              columns={[
                { key: 'exchange', label: 'Exchange', format: (v) => <span className="font-bold text-white capitalize">{v}</span> },
        ...
        <div>
          <h3 className="text-xl font-black uppercase tracking-tighter text-white mb-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="w-2 h-2 bg-[#00d672] rounded-full animate-pulse" /> Live Funding Rates
            </div>
            <span className="text-[9px] text-[#00d672] font-mono tracking-widest bg-[#00d672]/10 border border-[#00d672]/30 px-2 py-1 rounded">Binance</span>
          </h3>
          <div className="border border-[#1a1a1a] bg-[#0a0a0a]">
            <DataTable
              columns={[
                { key: 'symbol', label: 'Pair', format: (v) => <span className="font-bold text-white">{v}</span> },
                ...
                { 
                  ...
                  format: (v) => {
                    const num = Number(v);
                    const color = num > 0.01 ? 'text-[#00d672]' : num < -0.01 ? 'text-[#ff4757]' : 'text-[#888]';
                    return <span className={`${color} font-mono font-bold`}>{num > 0 ? '+' : ''}{num.toFixed(4)}%</span>;
                  }, 
                  align: 'right' 
                },
              ]}
