...
// Display range: 0 → 50× (Thermocap Multiple rarely exceeds 40 at cycle tops)
const TC_MAX = 50;

// Historically calibrated zone thresholds (MarketCap ÷ CumulativeThermocap)
// < 5   → Undervalued (deep bear)
// 5–15  → Fair Value
// 15–30 → Overvalued
// > 30  → Extreme (cycle top territory)
function tcColor(v: number): string {
  if (v < 5)  return "#00d672";
  if (v < 15) return "#FABF2C";
  if (v < 30) return "#f97316";
  return "#ff4d4f";
}

function tcZoneLabel(v: number): string {
  if (v < 5)  return "Undervalued";
  if (v < 15) return "Fair Value";
  if (v < 30) return "Overvalued";
  return "Extreme";
}
...
const LEGEND = [
  { range: "< 5×",   label: "Undervalued", color: "#00d672" },
  { range: "5–15×",  label: "Fair Value",  color: "#FABF2C" },
  { range: "15–30×", label: "Overvalued",  color: "#f97316" },
  { range: "> 30×",  label: "Extreme",     color: "#ff4d4f" },
];
...
function TcTooltip({
  ...
  return (
    <div className="bg-[#0a0a0a] border border-[#1a1a1a] px-3 py-2 text-xs">
      ...
      <p className="font-mono font-black" style={{ color: tcColor(v) }}>
        {v.toFixed(2)}× — {tcZoneLabel(v)}
      </p>
    </div>
  );
}

export default function ThermocapGauge({ multiple, points, source }: Props) {
  ...
  const color     = tcColor(multiple);
  const zone      = tcZoneLabel(multiple);
  const needleDeg = (Math.min(Math.max(multiple, 0), TC_MAX) / TC_MAX) * 180 - 90;
    ...
    <div className="bg-[#0a0a0a] border border-[#1a1a1a] p-6">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          ...
          <p className="text-[10px] font-mono text-[#555] mt-1 pl-3">
            Market Cap ÷ Cumulative Miner Revenue · derived from blockchain.info{source === "live" ? " · Live" : " · Seed (Apr 2026)"} · Cached 24 h
          </p>
        </div>
        <span
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start">
        ...
        <div className="flex flex-col items-center shrink-0 md:w-44 pt-2">
          ...
          <p className="text-[9px] font-mono text-[#555] mt-1">thermocap multiple</p>
        </div>

        {/* ── Thermocap history area chart ─────────────────────────────────────── */}
          ...
          {mounted && points.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={points} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                ...
                />
                <ReferenceLine y={5}  stroke="#00d672" strokeWidth={0.5} strokeDasharray="2 2" opacity={0.6} label={{ value: "5×",  fill: "#00d672", fontSize: 8, position: "insideTopRight" }} />
                <ReferenceLine y={15} stroke="#FABF2C" strokeWidth={0.5} strokeDasharray="2 2" opacity={0.5} label={{ value: "15×", fill: "#FABF2C", fontSize: 8, position: "insideTopRight" }} />
                <ReferenceLine y={30} stroke="#f97316" strokeWidth={0.5} strokeDasharray="2 2" opacity={0.5} label={{ value: "30×", fill: "#f97316", fontSize: 8, position: "insideTopRight" }} />
                <Tooltip content={<TcTooltip />} />
                <Area
                  type="monotone"
                  dataKey="value"
                  stroke="#FABF2C"
